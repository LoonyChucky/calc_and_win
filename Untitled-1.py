...
class Mentor(Human):
    def answer_question(self, question):
        if question == f'{self.name}, мне грустненько, что делать?':
            print('Отдохни и возвращайся с вопросами по теории.')
        elif question == f'{self.name}, как устроиться работать питонистом?':
...
class CodeReviewer(Human):
    def answer_question(self, question):
        if question == f'{self.name}, что не так с моим проектом?':
